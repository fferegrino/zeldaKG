...
def format_results_as_markdown_table(results):
    ...
    for result in results[1:]:
        column_widths = [max(column_width, len(value)) for column_width, value in zip(column_widths, result)]
    ...
    def format_row(row, space_char=" "):
        return (
            "|"
            + ("|".join([f"{space_char}{value:<{column_widths[i]}}{space_char}" for i, value in enumerate(row)]))
            + "|"
        )
...
{{question}}
        ...
        completions = openai_client.chat.completions.create(
            model=os.environ["OPENAI_MODEL"], temperature=0.0, max_tokens=1000, messages=messages
        )
